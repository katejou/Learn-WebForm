<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CRYP_2.aspx.cs" Inherits="note3_ex3.ASYN_CRYP" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    <title></title>
</head>
<body>
    <form id="form1" runat="server">
        <div>
            <br/>
            TripleDESCryptoServiceProvider(對稱) + RSACryptoServiceProvider(非對稱)
            <br/>
            <asp:Button ID="Button1" runat="server" Text="加密" OnClick="Button1_Click" />
            <br/>
            <asp:Label ID="Label1" runat="server" Text=""></asp:Label>
            <br/>
            <asp:Button ID="Button2" runat="server" Text="解密" OnClick="Button2_Click" />
            <br/>
            <asp:Label ID="Label2" runat="server" Text=""></asp:Label>
            <br/>
            <hr/>
            <br/>
            DESCryptoServiceProvider + RSACryptoServiceProvider(非對稱)
            <br/>
            <asp:Button ID="Button3" runat="server" Text="加密" OnClick="Button3_Click" />
            <br/>
            <asp:Label ID="Label3" runat="server" Text=""></asp:Label>
            <br/>
            <asp:Button ID="Button4" runat="server" Text="解密" OnClick="Button4_Click" />
            <br/>
            <asp:Label ID="Label4" runat="server" Text=""></asp:Label>
            <br/>
        </div>
    </form>
</body>
</html>
